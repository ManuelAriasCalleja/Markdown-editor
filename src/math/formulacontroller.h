...
/// \file
/// \brief Inserción y edición de fórmulas TeX en el editor WYSIWYG y su protección frente al teclado.
...
/// \brief Inserción y edición de fórmulas TeX (`$...$` / `$$...$$`) renderizadas en el
...
public:
    /// \brief Construye el controlador sobre `editor`; `parent` es el padre de los diálogos.
    FormulaController(QTextEdit *editor, QWidget *parent);

    /// \brief Doble clic sobre una fórmula: si en `viewportPos` (coords del viewport del
    /// editor) hay una fórmula, abre el diálogo precargado y la sustituye.
    /// \return true si actuó (atendió el clic, aunque el usuario cancele).
    bool editFormulaAt(const QPoint &viewportPos);
    /// \brief Intercepta teclas cerca o dentro de una fórmula: bloquea el tecleo de
    /// imprimibles dentro (se editan con doble clic) y convierte Backspace/Delete en
    /// el borde en borrado atómico del grupo.
    /// \return true si consumió el evento.
    bool handleMathKeyPress(QKeyEvent *event);
    /// \brief Si la selección toca o atraviesa fórmulas, la extiende a los grupos enteros
    /// (para que un pegado reemplace la fórmula completa, no la parta).
    void guardPasteAgainstMath();

public slots:
    /// \brief Diálogo (Ctrl+Shift+F) e inserción de la fórmula en el cursor.
    void insertFormula();

signals:
    /// \brief Emite un mensaje para la barra de estado.
