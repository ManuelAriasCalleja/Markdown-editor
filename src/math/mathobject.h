...
/// \file
/// \brief QTextObjectInterface que mide y pinta las fórmulas 2D incrustadas en el documento.
...
/// \brief QTextObjectInterface que pinta una fórmula 2D (mdmath::MathObjectType) en el
...
public:
    ...
    /// \brief Tamaño intrínseco de la fórmula del fragmento en `posInDocument` (delega en measureFormula).
    QSizeF intrinsicSize(QTextDocument *doc, int posInDocument,
                         const QTextFormat &format) override;
    /// \brief Pinta la fórmula del fragmento dentro de `rect` (delega en paintFormula).
    void drawObject(QPainter *painter, const QRectF &rect, QTextDocument *doc,
                    int posInDocument, const QTextFormat &format) override;

    /// \brief Registra `handler` como responsable de MathObjectType en el layout de
