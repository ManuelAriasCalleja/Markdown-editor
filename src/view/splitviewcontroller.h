...
/// \file
/// \brief Vista del Markdown crudo: modo fuente, vista dividida y sincronización entre paneles.
...
public:
    ...
    /// Editor de fuente. MainWindow lo necesita para el zoom, el modo sin
    /// distracciones y el filtro de eventos.
    FocusEditor *sourceEditor() const { return m_sourceEditor; }
    /// \brief QSplitter central (para zoom y guardado del estado del divisor).
    QSplitter *splitView() const { return m_splitView; }

    /// Acciones de menú que reflejan/disparan los modos (se conectan a los toggles).
    /// Se fijan tras crear los menús.
    void setModeActions(QAction *sourceModeAction, QAction *splitAction);
    /// \brief Fija la lista de acciones válidas solo en WYSIWYG (se deshabilitan en fuente).
    ...
    /// \brief Indica si el modo fuente a pantalla completa está activo.
    bool sourceMode() const { return m_sourceMode; }
    /// \brief Indica si la vista dividida está activa.
    bool splitMode() const { return m_splitMode; }
    /// \brief Indica si el panel de fuente tiene cambios sin volcar al documento.
    bool isSourceDirty() const { return m_sourceDirty; }

    /// \brief Si el panel de fuente tiene cambios sin volcar, los aplica al documento.
    ...
    /// \return El valor previo del flag, que debe pasarse a endProgrammaticChange().
    bool beginProgrammaticChange();
    /// \brief Restaura el flag anti-bucle al valor devuelto por beginProgrammaticChange().
    void endProgrammaticChange(bool previous) { m_syncing = previous; }

public slots:
    /// \brief Entra/sale del modo fuente a pantalla completa.
...
signals:
    ...
    /// \brief El estado de las acciones de tabla debe refrescarse.
    void tableActionsShouldUpdate();
    /// \brief El estado de las acciones de formato/marcas debe refrescarse.
    void formatActionsShouldUpdate();
    /// \brief El documento se modificó (escritura en el panel de fuente).
