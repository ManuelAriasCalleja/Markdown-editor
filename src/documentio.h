...
// Lee y escribe el documento Markdown sobre un QTextEdit: gestiona la
...
public:
    ...
    // Carga `path` en el editor. Devuelve false y rellena *errorString si falla.
    bool load(const QString &path, QString *errorString);
    // Escribe el documento en `path`. Devuelve false y rellena *errorString.
    bool write(const QString &path, QString *errorString);

    QString currentFile() const;
    // Modificado = el contenido (Markdown) difiere del último guardado/cargado.
    ...
    // El bloque de front matter verbatim (delimitadores incluidos), o "". Útil
    // para leer metadatos como el idioma del documento al exportar.
    QString frontMatter() const;

signals:
    // El archivo asociado cambió (al cargar, guardar o empezar uno nuevo).
    void currentFileChanged(const QString &path);
    // Se acaba de cargar contenido (p. ej. para que la vista recoloree enlaces).
    void documentLoaded();
